...
const NAV = [
  ...
  { href: '/stock-management',label: 'Stock',     icon: 'stock'     },
];
...
export default function Sidebar({ onClose }) {
  ...
  const { tiktokConnected, shopeeConnected, tiktokShopName, tiktokSellerName } = useAuth();

  return (
    <aside className="flex h-full w-60 flex-col bg-sidebar select-none">
      {/* Logo */}
      <div className="flex items-center gap-3 px-5 py-5 border-b border-white/5">
        <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#0026CC] shadow-sm shrink-0">
      ...
      {/* Nav — usePathname replaces NavLink's isActive */}
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all ${
                isActive
                  ? 'bg-[#0026CC]/15 text-[#0026CC]'
                  : 'text-slate-400 hover:bg-white/5 hover:text-white'
              }`}
            >
              <span className={`flex h-5 w-5 items-center justify-center ${isActive ? 'text-[#0026CC]' : ''}`}>
      ...
      <div className="px-3 pb-2 space-y-2">
        ...
        <div className="flex items-center gap-2.5 rounded-xl px-3 py-2.5 bg-white/4 border border-white/5">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-medium text-slate-300 truncate">{tiktokShopName || tiktokSellerName || 'TikTok Shop'}</p>
            {tiktokConnected
